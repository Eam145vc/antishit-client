...
namespace AntiCheatClient.Core.Services
{
    public class MonitorService
    {
        ...
        private readonly MonitorDetector _monitorDetector;
        ...
        public async Task<bool> SendMonitorData()
        {
            try
            {
                ...
                // Recopilar información detallada de monitores
                List<MonitorInfo> monitors = _monitorDetector.DetectMonitors();

                // Agregar información de monitores al objeto SystemInfo
                systemInfo.ScreenResolution = GetCombinedResolution(monitors);
                systemInfo.MonitorsInfo = GetMonitorsInfoList(monitors);

                // Recopilar información de dispositivos USB
                List<DeviceInfo> usbDevices = GetConnectedDevices();

                // Agregar información de monitores a la lista de dispositivos
                foreach (var monitor in monitors)
                {
                    DeviceInfo monitorDevice = monitor.ToDeviceInfo();
                    // Añadir solo si no existe ya un dispositivo con el mismo ID
                    if (!usbDevices.Exists(d => d.DeviceId == monitorDevice.DeviceId))
                    {
                        usbDevices.Add(monitorDevice);
                    }
                }
                ...
                Console.WriteLine($"Enviando monitores: {monitors.Count} monitores");
        ...
        // Obtiene una cadena de texto con las resoluciones combinadas de todos los monitores
        private string GetCombinedResolution(List<MonitorInfo> monitors)
        {
            if (monitors == null || monitors.Count == 0)
            {
                return $"{System.Windows.SystemParameters.PrimaryScreenWidth}x{System.Windows.SystemParameters.PrimaryScreenHeight}";
            }

            List<string> resolutions = new List<string>();
            foreach (var monitor in monitors)
            {
                string resolution = monitor.GetResolutionString();
                if (!resolutions.Contains(resolution) && resolution != "Unknown Resolution")
                {
        ...
        // Obtiene una lista de información de monitores para incluir en SystemInfo
        private List<object> GetMonitorsInfoList(List<MonitorInfo> monitors)
        {
            ...
            foreach (var monitor in monitors)
            {
                result.Add(monitor.GetMonitorInfo());
            }
